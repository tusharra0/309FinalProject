...
const listUserTransactions = async ({ userId, page = 1, limit = 10, ...rest } = {}) => {
  const safePage = Math.max(Number(page) || 1, 1);
  const safeLimit = Math.min(Math.max(Number(limit) || 10, 1), 100);

  // Build filter conditions from provided query params (type, amount, name, etc.)
  const filtersWhere = buildTransactionsWhere(rest);

  // Ensure the results are limited to transactions the user is involved in.
  ...
  // Combine filters with user involvement. Using AND ensures both filter
  // constraints and user involvement are applied. This also preserves any
  // OR conditions generated by `buildTransactionsWhere` (e.g. name search).
  const where = {
    AND: [filtersWhere, userInvolvement]
  };

  const orderField = rest.orderBy || 'id';
  const orderDir = rest.order === 'asc' ? 'asc' : 'desc';
  const allowed = ['id', 'createdAt', 'pointsDelta', 'type'];

  const orderByObj = allowed.includes(orderField) ? { [orderField]: orderDir } : { id: 'asc' };
